...
func _on_entered(area) -> void:
	if area.is_in_group("AxeAttack"):
		...
		if area.owner.is_good_attack and area.owner._state != area.owner.AxeState.STUCK:
			await get_tree().create_timer(0.1).timeout
			destroy()
		elif area.owner._state == area.owner.AxeState.STUCK and area.owner.is_good_attack:
			set_process_input(true)
			area.owner.stuck_finished.connect(func(): 
				set_process(true)
				set_process_input(false)
				destroy()
				)
